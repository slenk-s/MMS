...
class ConfigView(QWidget):
    ...
    def _build_update_form(self, layout):
        ...
        # 发布更新按钮
        self.publish_btn = QPushButton(tr("update.publish"))
        self.publish_btn.setMinimumHeight(36)
        self.publish_btn.setStyleSheet(
            "QPushButton { background-color:#06b6d4; color:white; font-size:13px; font-weight:bold; "
            "border-radius:6px; padding:8px; }"
            "QPushButton:hover { background-color:#0891b2; }"
            "QPushButton:disabled { background-color:#94a3b8; }"
        )
        self.publish_btn.clicked.connect(self._on_publish_update)
        btn_row = QHBoxLayout()
        btn_row.addStretch()
    ...
    def _on_publish_update(self):
        ...
        from utils.updater import push_update
        from utils.ftp_config import load_update_config
        from widgets.toast import toast_success, toast_error

        self.publish_btn.setEnabled(False)
        dialog = QProgressDialog("正在打包并上传更新...", "取消", 0, 0)
        dialog.setWindowTitle("发布更新")
        ...
        def worker():
            try:
                ...
                if not ftp_cfg["host"]:
                    result_container["result"] = (False, "未配置 FTP 主机")
                    return
                result = push_update(ftp_cfg)
                ok, msg = result if isinstance(result, tuple) else (False, str(result))
                result_container["result"] = (ok, msg)
            except Exception as e:
                result_container["result"] = (False, str(e))
        ...
        def poll():
            if "result" in result_container:
                ok, msg = result_container["result"]
                dialog.close()
                dialog.deleteLater()
                timer.stop()
                timer.deleteLater()
                ...
                if ok:
                    toast_success(self, "更新发布成功: %s" % msg)
                else:
                    toast_error(self, "更新发布失败: %s" % msg)
                return
